...
export const useTodoStore = create<TodoStore>((set) => ({
  ...
  editTodo: (id, title, isCompleted) =>
    set((state) => ({
      todos: state.todos.map((todo) =>
        todo.id === id ? { ...todo, title: title, completed: isCompleted } : todo
      ),
    })),
  ...
  editTodoStep: (todoId: string, stepId: string, title: string, completed: boolean) =>
    set((state) => ({
      todos: state.todos.map((todo) =>
        todo.id === todoId
          ? {
  ...
  removeTodoStep: (id, step) =>
    set((state) => ({
      todos: state.todos.map((todo) =>
        todo.id === id ? { ...todo, steps: todo.steps.filter((s) => s !== step) } : todo
      ),
    })),
}));
