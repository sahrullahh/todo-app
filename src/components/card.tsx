    ...
    <>
      <div className="group flex w-full items-center gap-4 rounded-md bg-white p-2 shadow transition-all hover:border-r-8 hover:border-green-600 hover:bg-green-100/20">
              ...
              <span className="peer-checked:line-through">
                ...
                <div className="flex items-center gap-2">
                  <p className={`text-sm text-gray-500 ${steps.length > 0 ? 'block' : 'hidden'}`}>
                    {stepCompletedCount[id]} of {steps.length}
                  </p>
                  <p
                    className={`text-sm font-semibold text-red-500 ${steps.length > 0 ? 'block' : 'hidden'}`}
                  >
                    High
                  </p>
                  <p
                    className={`flex items-center justify-center gap-2 text-sm text-gray-500 ${steps.length > 0 ? 'block' : 'hidden'}`}
                  >
                    <Icon icon="material-symbols:circle" className="text-xs text-gray-200" />{' '}
                    <Icon icon="material-symbols:calendar-month-outline-rounded" /> Mon, Apr 25
                  </p>
                </div>
              </span>
            </>
        ...
        <div
          ...
          className={`w-full cursor-pointer p-4 ${isEditing ? 'hidden' : 'block'}`}
        ></div>
        <div className={`hidden items-center justify-end gap-2 pr-3 group-hover:flex`}>
          <button
            onClick={() => handleOpenSheet()}
